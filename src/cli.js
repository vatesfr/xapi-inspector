...
const COMMANDS = {
  async proxy (args) {
    let {
      bind = '0',
      _: [
        remote = requiredArg('remote address')
      ]
    } = minimist(args, {
      string: 'bind'
    })
    ...
    const handleRequest = async (req, res) => {
      ...
      await proxyHttpsRequest(remote, req)
        .pipe(res)
    }

    const handleXmlRpcRequest = async (req, res) => {
      const [ req1, req2 ] = createReadableCopies(2, req)
      const [ res1 ] = createReadableCopies(1, await proxyHttpsRequest({
        ...req,
        ...remote
      }, req1))
      res1.pipe(res)
    ...
    const server = createHttpServer(wrapAsync(
      (req, res) => isXmlRpcRequest(req)
